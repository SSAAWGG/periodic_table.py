...
periodic_table = {
  ...
  "Symbol":"H",
    ...
    118: {"symbol": "Og", "name": "Oganesson", "mass": 2



    def get_by_number(number):
    return periodic_table.get(number, "Element not found")
...
# By approximate mass (within a range)
def get_by_mass(target_mass, tolerance=0.1):
    results = {}
    for num, data in periodic_table.items():
        if abs(data["mass"] - target_mass) <= tolerance:
            results[num] = data
    return results if results else "No elements found in this mass range"

# ----------------------------
# EXAMPLES
# ----------------------------

print(get_by_number(79))       # Gold
print(get_by_symbol("O"))      # Oxygen
print(get_by_name("Carbon"))   # Carbon
print(get_by_mass(16, 0.5))    # Find elements with mass ~16
